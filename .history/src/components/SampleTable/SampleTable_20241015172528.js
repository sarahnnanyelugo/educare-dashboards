...
const generateTimetableData = (numEntries, studentIds) => {
    ...
    for (let i = 0; i < numEntries; i++) {
      timetable.push({
        ...
        subject: arrayElement(['English', 'Mathematics', 'Geography', 'Chemistry', 'Physics','Biology']), // Use arrayElement helper function,
        day: arrayElement(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']),
        time: `${faker.number.int({ min: 1, max: 12 })}:${faker.number.int({ min: 0, max: 59 })} ${arrayElement(['AM', 'PM'])}`, // Updated for faker v7+
      });
    }
